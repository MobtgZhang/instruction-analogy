# 合成类比数据集 Agent Workflow

## 概述

本工作流用于从文档中自动合成高质量的类比推理数据集，数据集格式为 `(query, think, answer)`。
数据集在**纵向**维度按照类比阶次划分，在**横向**维度按照 Bloom 认知分类法划分。
...
| 阶次 | 名称 | 描述 | 三元组结构 |
|------|------|------|------------|
| 一阶 | 实体类比 | 两个实体之间的直接相似或对应关系 | `(entity_A, relation, entity_B)` |
| 二阶 | 关系类比 | 两对实体关系之间的结构相似性 | `(r1(A,B), ≅, r2(C,D))` |
| 三阶 | 子图类比 | 两个子知识图谱之间的结构同构或相似 | `SubGraph_1 ≅ SubGraph_2` |

### 横向维度：Bloom 分类法

| 认知层次 | 类比类型 | 说明 |
|----------|----------|------|
| **Factual（事实性）** | 实体类比 | 对具体实体之间的相似或对应关系进行比较 |
| | 属性类比 | 对实体属性或特征的相似性进行比较 |
| | 时间类比 | 对事件或过程的时间顺序或持续性进行比较 |
| | 空间类比 | 对位置、结构或空间关系进行比较 |
| | 量化类比 | 对数量、度量或比例进行比较 |
| | 事件类比 | 对事件的发生、因果或序列进行比较 |
| **Conceptual（概念性）** | 关系类比 | 对概念间的关系模式进行类比 |
| | 系统类比 | 对复杂系统或结构的相似性进行比较 |
| | 功能类比 | 对不同对象或概念的功能作用进行比较 |
| | 因果类比 | 对因果关系模式进行类比 |
| | 比例类比 | 对数量或性质之间的比例关系进行比较 |
| | 隐喻/比喻类比 | 对抽象概念的比喻性或象征性相似性进行比较 |
...
HuggingFace 文档数据集
    ...
    ▼
┌──────────────────────────────────────────────────────────┐
│         Sub-Module 0: 知识图谱构建子工作流（KG Builder）   │
│                                                          │
│  Step-A 数据集下载与预处理                                │
│      │                                                   │
│      ▼                                                   │
│  Step-B 类比导向三元组抽取（六大关系类型）                 │
│      │                                                   │
│      ▼                                                   │
│  Step-C Web Search 验证 → 修正 / 置信度标注               │
│      │                                                   │
│      ▼                                                   │
│  知识图谱 G（高质量、可类比）                              │
└──────────────────────────────────────────────────────────┘
...
└─────────────────────────────────────────┘
    ...
    ▼
┌─────────────────────────────────────────┐
│   Sub-Module 2: 数据合成子工作流         │
│        (Agent A)                        │
│  (query, think, answer) 三元组生成       │
│  显式类比 + 隐式类比双模式               │
└─────────────────────────────────────────┘
    │
    ▼
┌─────────────────────────────────────────┐
│   Sub-Module 3: 质量验证子工作流         │
│        (Agent B)                        │
│  合法性验证 → 错误修正 → 重新生成        │
└─────────────────────────────────────────┘
    │
...
> 本子模块是整个流程的数据基础，独立封装为可复用的子工作流。
> 输入为 HuggingFace 上的公开文档数据集，输出为面向类比推理的结构化知识图谱 `G`。
...
│                            └───────────┘ (修正反馈回Step-B)          │
│                                                                      │
│  输出：知识图谱 G = (V, E, L)  +  三元组置信度表                     │
└──────────────────────────────────────────────────────────────────────┘
```

---

### Step-A：文档数据集收集 Agent

#### A.1 推荐 HuggingFace 数据集

以下数据集经过筛选，覆盖多领域、富含实体关系，适合构建面向类比推理的知识图谱：
...
## Phase 4：Bloom 分类与标注 Agent（Agent C）
...
本工作流在代码库中的对应实现如下，两处文档内容一致（中文见 `agent_workflow_CN.md`，英文见 `agent_workflow.md`）。

| 阶段 | 目录/入口 | 说明 |
|------|-----------|------|
| **Sub-Module 0** 知识图谱构建 | `AnalogyKG/` | 语料准备、类比三元组抽取（entity/relation/subgraph）、Web 验证与修正；入口 `build.py`，配置与 prompt 见 `config/`。 |
| **Sub-Module 2–4** 指令合成与验证 | `AnalogySyn/` | 读取类比数据 → 合成 LLM 训练指令 (input, thinking, output) → Bloom 分类 → 验证 → 不通过则修正；入口 `run.py`，三步逻辑分别位于 `synthesize.py`、`validate.py`、`correct.py`，配置与 prompt 见 `config/`。 |

数据流：`AnalogyKG` 输出 `entity/relation/subgraph_analogy_triples.jsonl` 至 `data_dir`，`AnalogySyn` 从该目录读取并生成微调用指令 `instructions.jsonl`。
